...
public class MarketService : IMarketService
{
    ...
    private readonly Product.Business.Interfaces.Notifications.IMarketNotifier _notifier;
    private readonly IWalletRepository _walletRepo;
    private readonly Product.Business.Interfaces.Categories.ICategoryService _categoryService;

    public MarketService(
        IMarketRepository marketRepo,
        AppDbContext db,
        Product.Business.Interfaces.Notifications.IMarketNotifier notifier,
        IWalletRepository walletRepo,
        Product.Business.Interfaces.Categories.ICategoryService categoryService
    )
    {
            ...
            else
            {
                ...
                q = q.Where(m =>
                    ...
                    || (
                        slugList.Count > 0
                        && (
                            (m.Category != null && slugList.Contains(m.Category))
    ...
    )
    {
        ...
        try
        {
            ...
            var ledgerEntry = new Product.Data.Models.Wallet.LedgerEntry
            {
                Id = Guid.NewGuid(),
                AccountId = account.Id,
                Amount = -net,
                Type = Product.Common.Enums.LedgerEntryType.BET_BUY,
            ...
            var txRecord = new Product.Data.Models.Markets.Transaction
            {
                Id = Guid.NewGuid(),
                UserId = userId,
                Type = "buy",
                ...
                NetAmount = net,
                MarketId = marketId,
                Description = $"Compra {contracts} contratos {side}",
            };
            ...
            // Persist immutable receipt for the buy (snapshot)
            try
            {
                var receipt = new Product.Data.Models.Wallet.Receipt
                {
                    ...
                    Currency = "BRL",
                    ReferenceType = "MarketTransaction",
                    ReferenceId = txRecord.Id,
                    PayloadJson = JsonSerializer.Serialize(new { tx = txRecord, ledger = ledgerEntry, marketId = marketId, positionId = pos.Id })
                };
                await _walletRepo.AddReceiptAsync(receipt, ct);
            }
            catch
            {
                // do not break buy flow if receipt persistence fails
            }
